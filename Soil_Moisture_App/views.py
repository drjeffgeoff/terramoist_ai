    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Registration successful! {
                             username}, welcome to our community.')
...
def predict(request):
    ...
    if request.method == 'POST':
        ...
        model = joblib.load(
            r'C:\Users\user\Desktop\Recess2 Project\SoilMoistureMonitorApp\random_forest_model.pkl')
        scaler = joblib.load(
            r'C:\Users\user\Desktop\Recess2 Project\SoilMoistureMonitorApp\randomScaler.pkl')
        ...
        Notification.objects.create(
            user=request.user,
            message=f'New soil moisture reading recorded: {
                comment} at {location} on {date} {time}.'
        )

        # Add success flag to context
        context['success_message'] = f'Prediction recorded successfully: {
            comment}'
...
def current_predictions(request):
    ...
    for field in fields:
        try:
            latest_entry = SoilData.objects.filter(
                location=field, date=today).order_by('-date', '-time').first()
...
def chart(request):
   # Fetch soil moisture data
    ...
    avg_rainfalls = [round(item['avg_rainfall'], 2)
                     for item in average_rainfall_per_area]
